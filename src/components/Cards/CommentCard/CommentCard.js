...
const Avatar = ({ avatar }) => {
  ...
  return(
    <Col xs={4} className="comment-avatar">
      <Image roundedCircle src={avatar ? avatar : defaultAvatar} className="avatar-style" style={avatarStyle} />
    </Col>
  )
}
  ...
  return(
    <Col  xs={8}>
      <ListGroup style={{ display: "flex", rowGap: "15px" }}>
        <span>{message}</span>
        <span>{date}</span>
        <Button style={{ fontSize: "inherit", width: "fit-content", alignSelf: "self-end", backgroundColor: "#FFF", color: "#000", border: "#2C2F35 1px solid" }}>Go to the article</Button>
      </ListGroup>
